...
let timerInterval = null; export default function Buzzer(props) {
  ...
  const handleKeyPress = (event) => {
    if (event.key === " " && !buzzed) {
      ...
      setKeysPressed((prevState) =>
          Object.assign({}, prevState, { [event.key]: true })
      );
    }
  };
  ...
  const handleKeyRelease = (event) => {
    if (event.key === " " && buzzed) {
      ...
      let { [event.key]: key, ...rest } = keysPressed;
      setKeysPressed(rest);
    }
  };
